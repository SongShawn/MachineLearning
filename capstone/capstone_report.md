...
随机森林选用常用参数直接训练，参数设置："n_estimators": 500, "max_depth": 8, "min_samples_split": 20, max_features: 0.8，其余参数均是缺省值。最终得到的交叉验证损失值为：

XGBoost开始使用gpu_hist算法寻找分裂点，但是训练处的模型对测试集进行与测试总是出现异常退出。没有精力定位问题，因此改用基于CPU的hist算法进行训练。刚开始训练时，选用了较小的学习速率0.01，并将迭代轮数设置为5000，early_stop设置为20，得到了较小的验证集损失值，但是在测试集上得到远高于验证集的损失值，出现明显的过拟合现象。

增大学习速率为0.1，降低迭代轮数为3000，以如下参数训练XGBoost模型：
- max_depth: 6
- sumsample: 0.5
...
由于训练集中存在一个类别的样本数过少，数量仅为6，但是鉴于训练集整体样本数很大，因此采用5折交叉验证和网格搜索法寻找随机森林最优的超参数。 
需要遍历的超参数和对应的数值如下：
...
如果直接按照上述网格参数，再结合10折交叉验证，一共需要训练`16800`(7 * 2 * 5 * 4 * 6 * 2 * 5)个模型。按照10秒钟训练一个模型，将需要将近100个小时，我们无法承受这个时间消耗，因此改变为如下近似策略。

n_estimators和max_depth有一定的关联性，因此这两个参数同时进行网格搜索。其它参数每次仅尝试搜索上述一个参数，将得到的最优参数作为结果。每次进行网格搜索时，除目标参数外，已经找到最优参数的直接使用，还没有找到的随心填写一个即可，或者直接使用缺省值。在这种策略下，计算模型个数公式中的乘法便变成了加法(最后一个乘号不变)。仅仅需要训练`245`((7 * 5 + 2 + 4 + 6 + 2) * 150)个模型，大大缩短了训练时间。
