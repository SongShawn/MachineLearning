...
class Runner(object):
    ...
    def run_training(self, training_epoch, training_per_epoch=150, display_direction=False):
        ...
        for e in range(training_epoch):
            ...
            for i in range(training_per_epoch):
                ...
                action, reward = self.robot.update(np.mean(self.train_robot_statics['times'][-a:]))
    ...
    def run_testing(self, testing_per_epoch):
        ...
        testing_per_epoch = max(int(self.maze.height * self.maze.height * 0.85),testing_per_epoch)
        accumulated_reward = 0.
        run_time = 0
    ...
    def write_on_header(self, header_img, parameters):
        ...
        line2 = ',\n'.join([a + ":{}".format(q) for a,q in parameters['qtable'].items()])
        if not parameters['success']:
            line3 = "\nAction %s, Reward %f \n"%(parameters['action'],parameters['reward'])
            line4 = "Eps: %.2f, Alp: %.2f, Gam: %.2f"%(parameters['epsilon'],parameters['alpha'],parameters['gamma'])
        else:
            line3, line4 = '', ''
        draw.text((230, 85), line0, (0,0,0), font=self.header_font)
        draw.text((480, 0), line1+line2+line3+line4, (0,0,0), font=self.font)
    ...
    def info_to_file(self, file_name):
        f = open(file_name, 'w') 
        for k,v in self.train_robot_statics.items():
            f.write('{}: {}\n'.format(k, v))
        f.close()