...
class Maze(object):
    ...
    def maze_to_file(self, map_file, dest_file, trap_file):
        f = open(map_file, 'w')
        str_maze_data = self.maze_data.astype(str)
        for row_data in str_maze_data:
            f.write(','.join(row_data))
            f.write('\n')
        f.close()

        f = open(dest_file, 'w')
        f.write(','.join([str(i) for i in self.destination]))
        f.close()

        f = open(trap_file, 'w')
        for trap in self.__traps:
            f.write(','.join([str(i) for i in trap]))
            f.write('\n')
        f.close()
